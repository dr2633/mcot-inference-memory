...
def main():
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        json.dump(all_temp_stats, f, indent=2)
        ...
        for i in range(len(values["output_tokens_list"])):
            try:
                plot_data.append({
                    ...
                    "Semantic Similarity": values["similarity_list"][i],
    ...
    # 2. Scatter Plot: Semantic Similarity vs Temperature
    plt.figure(figsize=(10, 6))
    ...
    plt.plot(list(accuracy_data.keys()), list(accuracy_data.values()), 'o-', linewidth=2)
    ...
    plt.bar(list(steps_data.keys()), list(steps_data.values()), color='skyblue')
    plt.xlabel("Temperature")
